...
void DialogWindow::DoDialog(const char *text, int yTop, int ySize)
{
 ...
 //   System::paintSync();
    ...
    GetCube()->backgroundLayer.set();
    GetCube()->backgroundLayer.setWindow(0, 128);
    GetCube()->foregroundLayer.Clear();
    GetCube()->GetView()->Paint();
    ...
    GetCube()->vbuf.poke(offsetof(_SYSVideoRAM, colormap) / 2 + 0, fg);
    GetCube()->vbuf.poke(offsetof(_SYSVideoRAM, colormap) / 2 + 1, bg);
    GetCube()->vbuf.pokeb(offsetof(_SYSVideoRAM, mode), _SYS_VM_FB128);
    GetCube()->backgroundLayer.setWindow(yTop, ySize);
