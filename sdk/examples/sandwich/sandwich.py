...
class World:
	...
	def export(self):
		with open(os.path.join(self.dir,"gen_assets.lua"), "w") as lua:
		...
		with open(os.path.join(self.dir,"gen_assets.h"), "w") as hdr:
			hdr.write("// GENERATED BY SANDWICH.PY, DO NOT EDIT BY HAND\n")
			hdr.write("#pragma once\n")
			hdr.write("#include \"MapData.h\"\n\n")
			for map in self.maps.itervalues():
				map.write_header_to(hdr)
			hdr.write("\n")

		
		with open(os.path.join(self.dir,"gen_assets.cpp"), "w") as src:
			src.write("// GENERATED BY SANDWICH.PY, NO NOT EDIT BY HAND\n")
			src.write("#include \"gen_assets.h\"\n")
			src.write("#include \"game.h\"\n\n")
			for map in self.maps.itervalues():
...
class Map:
	...
	def write_header_to(self, hdr):
		hdr.write("extern MapData %s_data;\n" % self.name)

	def write_source_to(self, src):
		src.write("//--------------------------------------------------------\n")
		src.write("// EXPORTED FROM %s.tmx\n" % self.name)
		src.write("//--------------------------------------------------------\n\n")
		src.write("static uint8_t %s_xportals[] = {\n" % self.name)
		for y in range(self.height):
			src.write("    ")
			for x in range(self.width):
			...
			src.write(",\n")
		src.write("};\n")
		src.write("static uint8_t %s_yportals[] = {\n" % self.name)
		for x in range(self.width):
			src.write("    ")
			for y in range(self.height):
		...
		if self.overlay is not None:
			for room in self.rooms:
				if room.hasoverlay():
					src.write("static uint8_t %s_overlay_%d_%d[] = {\n" % (self.name, room.x, room.y))
					src.write("    ")
					for y in range(8):
						for x in range(8):
					...
					src.write("0xff \n};\n")
		src.write("static RoomData %s_rooms[] = {\n" % self.name)
		for y in range(self.height):
			for x in range(self.width):
		...
		if self.overlay is not None:
			...
			overlay_msg = "0"
		src.write(
			"MapData %s_data = { &TileSet_%s, %s, &Blank_%s, %s_rooms, %s_xportals, %s_yportals, 0, 0, %d, %d };\n\n" % \
			(self.name, self.name, overlay_msg, self.name, self.name, self.name, self.name, self.width, self.height))
...
class Room:
	...
	def write_source_to(self, src):
		...
		# overlay
		if self.hasoverlay():
			src.write("        %s_overlay_%d_%d\n" % (self.map.name, self.x, self.y))
		else:
			src.write("        0\n")
		src.write("    },\n")		
