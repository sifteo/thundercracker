	...
	def export(self):
		with open("content.gen.lua", "w") as lua:
			...
			for filename in set((m.background_id for m in self.maps.maps)):
				lua.write("TileSet_%s = image{ \"%s\", width=16, height=16, flat=1 }\n" % (
			...
			for filename in set((m.overlay_id for m in self.maps.maps if m.overlay is not None)):
				lua.write("Overlay_%s = image{ \"%s\", width=16, height=16, flat=1 }\n" % (
					posixpath.splitext(filename)[0],
					posixpath.join(self.dir, filename)
				))
			...
			for name in self.dialogs.list_detail_image_names():
				lua.write("NPC_Detail_%s = image{ \"%s.png\", flat=1 }\n" % (name,posixpath.join(self.dir, name)))
