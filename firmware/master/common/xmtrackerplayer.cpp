...
bool XmTrackerPlayer::isUsingChannel(unsigned ch) const
{
    ...
    if (hasSong) {
        return ch >= ((unsigned)_SYS_AUDIO_MAX_CHANNELS - song.nChannels);
    }
...
void XmTrackerPlayer::processVolume(XmTrackerChannel &channel)
{
    ...
    switch (command) {
        case vxSlideDown:
            if (!param) {
                LOG(("%s:%d: NOT_IMPLEMENTED: empty param to vxSlideDown.\n", __FILE__, __LINE__));
            }
        ...
        case vxSlideUp:
            if (!param) {
                LOG(("%s:%d: NOT_IMPLEMENTED: empty param to vxSlideUp.\n", __FILE__, __LINE__));
            }
        ...
        case vxFineVolumeDown:
            if (ticks) break;
            if (!param) {
                LOG(("%s:%d: NOT_IMPLEMENTED: empty param to vxFineVolumeDown.\n", __FILE__, __LINE__));
            }
