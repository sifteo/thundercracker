...
void Group::getFlashCRC(std::vector<uint8_t> &crcbuf, unsigned addr, unsigned numBlocks) const
{
    /*
     * Pre-calculate the CRC for a number of blocks within this Group.
     *
     ...
     *
     * Defaults to CRCing a single block to minimize work for the cube at runtime.
     */
    ...
    numBlocks = std::min(numBlocks, pool.size() / TILES_PER_BLOCK);
    ...
    for (unsigned i = 0; i < numBlocks; ++i) {
        ...
        for (unsigned tile = 0; tile < TILES_PER_BLOCK; ++tile) {
            for (unsigned sample = 0; sample < 4; ++sample) {
                ...
                /*
                 * Locate the byte of data within this pool that
                 * corresponds to addr.
                 */
                TilePool::Index ti = (addr / sizeof(RGB565)) / Tile::PIXELS;
                unsigned pi = (addr / sizeof(RGB565)) % Tile::PIXELS;
                RGB565 px = pool.tile(ti)->pixel(pi);
                uint8_t byte = (px.value >> ((addr & 1) ? 8 : 0)) & 0xff;

                crc = gf84[1 + crc] ^ byte;
                addr = (addr & 0xFFFFFF80) | (crc >> 1);
            }
