 ...
 *     is not an argument, so check()'s error message would not make sense.
 */
...
public:
  ...
  static T *cast(lua_State *L, int idx) {
    userdataType *p = static_cast<userdataType*>(lua_touserdata(L, idx));
    ...
    lua_getfield(L, LUA_REGISTRYINDEX, T::className);
