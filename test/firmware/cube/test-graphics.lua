...
TestGraphics = {}
    ...
    function TestGraphics:test_stamp()
        -- Known background

        gx:setMode(VM_SOLID)
        gx:setColors{0x1234}
        gx:drawAndAssert("solid-1234")
        
        -- 6x4 pattern, with color key
        ...
        gx:drawAndAssert("stamp-6x4-key1")

        -- Disable color key

        gx.cube:xbPoke(0x324, 16)
        gx:drawAndAssert("stamp-6x4-nokey")

        -- Same pattern, different shape, different palette, partial-screen rendering.
        -- This one has width < 2*pitch.
        ...
        gx:setWindow(12, 31)
        gx:drawAndAssert("stamp-12x2-win")

        -- Now draw the "Mr. Pink" monster sprite using this mode,
        ...
        gx:setWindow(96, 32)
        gx:drawAndAssert("stamp-mrpink-bl")

        -- Combine with rotation

        for flags = 0, 7, 1 do        
            gx:xbReplace(VA_FLAGS, 5, 3, flags)
            gx:drawAndAssert(string.format("stamp-mrpink-bl-rotate-%d", flags))
        end
